...
def example_direct():
    """ Prints the list directly will provide a result
...
# this is how the list is displayed without the brackets and commas
def example_for():
...
def single_item():
    """ To get a single item from a list, you use a bracket
...
def slice_list():
    """ Slicing will provide a range of values from the list.
...
def negative_slice():
    ...
    """
    examples2 = my_list[-4:-2]
    print(examples2)
    for value in examples2:
        print(value)
...
# you can also use in operator instead of == in the if clause
def while_demo():
    # initialized variable below
    count = 0
    while count < len(trucks):
        if 'dodge' == trucks[count]:
            print('Dodge is Ram')
        else:
            print(trucks[count])
        count += 1
...
def example_pop():
    ...
    index is supplied. Otherwise, with the index, it
...
def example_clear():
    """ The clear method will remove all items from the
